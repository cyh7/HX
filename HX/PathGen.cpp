...
void PathGen::GenerateActualPaths(actual_path& ap)
{
	...
	else
	{
		...
		const int file_rsy = bp_info.min_height;
		//打开文件
		FILE* fp_output = NULL;
		...
		{
#ifdef  _DEBUG
			fprintf(fp_output, "%d %d %d\n", final_paths_[i].ex, final_paths_[i].ey, 1);
			const int next_index = (i + 1 == final_paths_.size()) ? 0 : i + 1;
			fprintf(fp_output, "%d %d %d\n", final_paths_[next_index].sx, final_paths_[next_index].sy, 0);
			...
			ap.real_path.push_back(final_paths_[i]);
			const int next_index = (i + 1l == final_paths_.size()) ? 0 : i + 1;
